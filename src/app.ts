...
const shouldLogin = async (page: Page, html: string) => {
    ...
    if (loginText.length > 0) {
        ...
        // await page.evaluate(() => {
        //     fetch('https://51fengliu.com/login.html', {
        //         method: 'POST',
        //         body: JSON.stringify({
        //             username: username,
        //             password: password
...
const fetchDetailData = async (page: Page, id: string) => {
    try {
...
const fetchPageData = async (page: Page, city: string, pageNo: number) => {
    try {
        await page.evaluate((cityCode: string, pageNo: number) => {
            try {
                // fetch("https://51fengliu.com/api/web/info/page.json?sort=publish&cityCode=110106&page=5", {
                //     "headers": {
                //       "accept": "application/json, text/plain, */*",
                //       "accept-language": "en-US,en;q=0.9,ko;q=0.8,pl;q=0.7,ru;q=0.6,zh-CN;q=0.5,zh;q=0.4",
                //       "Referer": "https://51fengliu.com/list?cityCode=110106&page=5",
                //       "Referrer-Policy": "strict-origin-when-cross-origin"
                //     },
                //     "body": null,
                //     "method": "GET"
                //   });
                //     https://51fengliu.com/api/web/info/page.json?sort=publish&cityCode=110106&page=5
        ...
        for (const province in cities) {
            ...
            for (const city in cities[province]) {
                ...
                while(true) {
                    try {
                        const time = +new Date()
                        const {pages, records, total} = await fetchPageData(page, city, 1)
